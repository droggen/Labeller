...
/* TODO:
    Should skip space/tab after comma as well
*/
...
*/
int parse(char *data,unsigned *sx,unsigned *sy,double **outdata,bool **outnan,unsigned fixedcol)
{
    unsigned idx=0;
    unsigned idx2;
    char t;
    ...
    while(1)    // byte at data[len] is 0, guaranteed by the callee.
    {
        ...
        if(data[idx]==',' || data[idx]==';')
        {
            ...
            if(expectdata)
            {
                v = 0.0;
                nan=true;
                if(fixedcol==0 || (fixedcol!=0 && col<fixedcol))		// Add number only if fixedcol is not specified or if the number ends in the first fixedcol
                {
        ...
        else if((data[idx]>='0' && data[idx]<='9') || data[idx]=='-'  || data[idx]=='.' || data[idx]=='N' || data[idx]=='n')
        {
            ...
            idx2=idx;
            while(data[idx2]!=0 && data[idx2]!=' ' && data[idx2]!='\t' && data[idx2]!=',' && data[idx2]!=';' && data[idx2]!=13 && data[idx2]!=10)
                idx2++;
        ...
        {
            //printf("newline/end: expectdata: %d col: %d\n",expectdata,col);
            ...
            if(fixedcol && col!=0)	// If fixed col, we must add as many numbers as columns, but only if that line contains any column in the first place
            {
                //printf("Should pad to %d\n",fixedcol);
                for(unsigned c=col;c<fixedcol;c++)
                {
                    v = 0.0;
                    nan=true;
                    if(!fastarrayadd(bufferval,sizeof(double),&v))
            ...
            if(gotlinedata==true)
            {
                ...
                else
                {
                    ...
                    // Check number of columns only if not fixed size
                    if(numcol != col)
                    {
                        errcode=1;
                goto enderror;
...
/*
    Returns:
        ...
        outnan:     pointer to allocated block of memory - must be freed by user
        outdata:    pointer to allocated block of memory - must be freed by user

    Data is organised in memory with columns in sequential memory order.
    Data is accessed as follows: outdata[y*sx+x]
