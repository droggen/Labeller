...
//		nsonscreen=np;
   ...
   if(EnableFrame)
   {
    ...
    // If the vertical mark bar is active, check whether visible and draw it
    if(markv_enable)
    {
        //printf("Draw vertical bar at %d color: %d\n",markv_location,markv_color);
        if(markv_location>leftsample+1 && markv_location<rightsample-1)
        {
            ...
            //printf(" vbar x is: %d\n",x);
            // Draw vertical bar
            fastvLineColor(x-1,0,h-1,markv_color);
            fastvLineColor(x+1,0,h-1,markv_color);
        }
    }
...
void Scope::SetVerticalMarkerBar(bool _markv_enable,int _markv_location,int _markv_color)
{
