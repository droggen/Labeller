...
private:
    ...
    protected:
      int basex,basey;                 // Offset of the drawing area
      bool highlight;
      string title;
      bool EnableDrawHGrid,EnableDrawVGrid,EnableFrame;
      unsigned np;                     // Internally used by Plot and DrawHAxis
      int leftsample, rightsample;     // Internally used by Plot and DrawHAxis
      bool gridcenter;                 // Origin of grid (gridcenter)
      bool RightAlignLast;             // When scope is not centered (right aligned), indicates whether the rightmost pixel is the last one in the data, or the user provided offset
    ...
    public:
      ...
      virtual void SetGridPosition(bool _center=false);
      virtual void SetRightAlignLast(bool _ralign=true);

      virtual void SetEnableVGrid(bool e);
      virtual void SetEnableHGrid(bool e);
      virtual void SetEnableFrame(bool e);
