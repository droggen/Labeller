...
protected:
   ...
   virtual void focusInEvent ( QFocusEvent * event ) {(void) event; printf("QScrollAreaExt focus in %p\n",this);}
   virtual void focusOutEvent ( QFocusEvent * event ) {(void) event; printf("QScrollAreaExt focus out %p\n",this);}
   //virtual void wheelEvent(QWheelEvent *evt) {printf("QScrollAreaExt::wheelEvent\n"); QScrollArea::wheelEvent(evt);}

   virtual void wheelEvent(QWheelEvent *event) {(void) event; printf("QScrollAreaExt::wheelEvent %p\n",this);}    // This is important: it avoids the wheel event to be dispatched to the scrollbar, and thus the scrollarea to scroll when the mouse is in the scope widget
